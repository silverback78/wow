...
function Druid:Restoration()
	...
	if talents[RO.CenarionWard] then
        MaxDps:GlowCooldown(RO.CenarionWard, cooldown[RO.CenarionWard].ready);
    end
	
	MaxDps:GlowCooldown(RO.SwiftMend, cooldown[RO.SwiftMend].ready);
	MaxDps:GlowCooldown(RO.Overgrowth, cooldown[RO.Overgrowth].ready);
	MaxDps:GlowCooldown(RO.NaturesSwiftness, cooldown[RO.NaturesSwiftness].ready and buff[RO.SoulOfTheForest].up);
	MaxDps:GlowCooldown(RO.AdaptiveSwarm, cooldown[RO.AdaptiveSwarm].ready);
	
	if debuff[RO.SunfireDebuff].refreshable then
        return RO.Sunfire;
    end
	
    if debuff[RO.MoonfireDebuff].refreshable then
        return RO.Moonfire;
    end
